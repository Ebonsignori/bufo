...
# Get tadpole ports (api and app) for tadpole N
...
sync_env_files() {
  ...
  for ((i=0; i<env_files_count; i++)); do
    ...
    # Copy from main repo if tadpole .env doesn't exist yet
...
# Kill processes running in a tadpole
# Two-phase approach:
#   Phase 1: Use cleanup.kill_pattern to find processes by tadpole directory name
...
kill_workspace_ports() {
  ...
  # Extract tadpole number from directory name
  local num=""
  ...
  # --- Phase 1.5: kill by tadpole working directory ---
  # Find server processes whose cwd is inside the tadpole directory.
  ...
  if [ -d "$dir" ]; then
    ...
    for pid in $listen_pids; do
      ...
      # Check if this process's cwd is in the tadpole
  ...
  # Strategy A: env_sync managed ports from tadpole .env files
  local env_files_count=$(yq -r '.env_sync.files | length // 0' "$CONFIG_FILE" 2>/dev/null)
  for ((i=0; i<env_files_count; i++)); do
    local env_path=$(yq -r ".env_sync.files[$i].path" "$CONFIG_FILE" 2>/dev/null)
    { [ -z "$env_path" ] || [ "$env_path" = "null" ]; } && continue

    # Check tadpole .env, fall back to main repo .env for base ports
    local full_path="$dir/$env_path"
  ...
  for ((i=1; i<=WORKSPACE_COUNT; i++)); do
    ...
    if lsof -i ":$api_port" &>/dev/null; then
      ...
      if [[ "$cwd" == *"$WORKSPACE_PREFIX-$i"* ]]; then
        ...
        local other_ws=$(echo "$cwd" | grep -oE "${WORKSPACE_PREFIX}-[0-9]+" | sed "s/${WORKSPACE_PREFIX}-/tp/")
