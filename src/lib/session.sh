...
session_create() {
  ...
  if [ -d "$session_dir" ]; then
    ...
    echo "Use 'bufo session resume $name' to continue it"
    return 1
  fi
...
session_update() {
  ...
  FIELD="$field" VALUE="$value" yq -i '.[strenv(FIELD)] = strenv(VALUE)' "$session_file"
}
...
session_list() {
  ...
  if [ ! -d "$sessions_dir" ]; then
    ...
    echo "Start one with: bufo session start \"name\""
    return
  fi
  ...
  for session_dir in "$sessions_dir"/*/; do
    ...
    if [ -n "$last_accessed" ] && [ "$last_accessed" != "null" ]; then
      local timestamp
      # BSD date (macOS): -j skips setting the clock, -f specifies input format
      # GNU date (Linux): uses -d for input string parsing
      timestamp=$(date -j -f "%Y-%m-%dT%H:%M:%SZ" "$last_accessed" "+%s" 2>/dev/null) \
        || timestamp=$(date -d "$last_accessed" "+%s" 2>/dev/null) \
        || timestamp=""
      if [ -n "$timestamp" ]; then
        local now=$(date "+%s")
...
# Write .bufo-meta to a session directory for the info bar
...
_write_session_meta() {
  ...
  local meta_file="$session_dir/.bufo-meta"
  ...
  # Add empty links array (no tadpole-level config links for sessions)
...
_open_session_layout() {
  ...
  create_tadpole_layout "$name" "$session_dir" "$server_cmd" "$claude_cmd" "" "new" ""
...
session_start() {
  ...
  local session_dir
  session_dir=$(session_create "$name" "$context") || return 1
...
session_resume() {
  ...
  if [ ! -f "$session_file" ]; then
    error "Session '$name' not found"
    echo "Use 'bufo session ls' to see available sessions"
    return 1
  fi
  ...
  local claude_cmd="$(get_ai_interactive_cmd "$name") --continue"
  _open_session_layout "$name" "$session_dir" "$claude_cmd" "" "resume"
}

# Close session iTerm2 tab if the layout is still live
# Usage: _close_session_panes <session_dir>
_close_session_panes() {
  local session_dir="$1"
  if _load_session_layout "$session_dir"; then
    local close_sid="${SESSION_INFO_SID:-$SESSION_MAIN_SID}"
...
session_delete() {
  ...
  if [[ "$confirm" =~ ^[Yy]$ ]]; then
    _close_session_panes "$session_dir"
    rm -rf "$session_dir"
...
handle_session_command() {
  ...
  case "$cmd" in
    ...
    "resume"|"continue")
      local name="${1:-}"
      if [ -z "$name" ]; then
        error "Specify session name: bufo session resume <name>"
        return 1
      fi
      session_resume "$name"
      ;;
    "delete"|"rm")
      local name="${1:-}"
      if [ -z "$name" ]; then
        error "Specify session name: bufo session delete <name>"
        return 1
      fi
      session_delete "$name"
      ;;
    "summary")
      ...
      local summary="${2:-}"
      if [ -z "$name" ]; then
        error "Usage: bufo session summary <name> \"summary text\""
        return 1
      fi
      if [ -z "$summary" ]; then
    ...
    *)
      ...
      echo "Usage: bufo session [ls|start|resume|delete|summary]"
      return 1
      ;;
  esac
}
