#!/usr/bin/env bash
# bufo - PR review and court review commands
...
get_court_instructions() {
  # Customize: ~/.bufo/prompts/court-judge.md
  # Note: The Reviewer A sub-prompt is embedded in this template.
  local default
  IFS= read -r -d '' default << 'COURT_EOF' || true
...
COURT_EOF
  load_prompt "court-judge" "$default"
}
...
EOF
  ...
  if [[ ! "$start_now" =~ ^[Nn]$ ]]; then
    ...
    local ai_cmd; ai_cmd="$(get_ai_interactive_cmd) 'context.md'"
...
# Show fun chorus intro animation
_show_chorus_intro() {
  ...
  echo ""
  # FIGlet-style banner - "BUFO CHORUS" (no box - cleaner look)
  echo -e "${C1}██████╗ ██╗   ██╗███████╗ ██████╗     ██████╗██╗  ██╗ ██████╗ ██████╗ ██╗   ██╗███████╗${RST}"
  echo -e "${C2}██╔══██╗██║   ██║██╔════╝██╔═══██╗   ██╔════╝██║  ██║██╔═══██╗██╔══██╗██║   ██║██╔════╝${RST}"
  echo -e "${C2}██████╔╝██║   ██║█████╗  ██║   ██║   ██║     ███████║██║   ██║██████╔╝██║   ██║███████╗${RST}"
  echo -e "${C3}██╔══██╗██║   ██║██╔══╝  ██║   ██║   ██║     ██╔══██║██║   ██║██╔══██╗██║   ██║╚════██║${RST}"
  ...
  echo -e "${C4}╚═════╝  ╚═════╝ ╚═╝      ╚═════╝     ╚═════╝╚═╝  ╚═╝ ╚═════╝ ╚═════╝ ╚═════╝ ╚══════╝${RST}"
  echo ""
  # Toad conductor with baton
  echo -e "                                      ${C2} ___  ${RST}"
  echo -e "                                     ${C2}(o o) ${RST}"
  echo -e "                                    ${C3}/|   |\\\\${RST}  ${C1}♪${RST}"
  echo -e "                                   ${C4} |___|  ${RST}"
  echo ""
...
EOF
  ...
  local codex_cmd
  codex_cmd=$(_build_reviewer_b_cmd)

  # Start AI tool as the judge in iTerm2 layout (with Codex in server pane)
  session_update "$name" "last_accessed" "$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
  local ai_cmd; ai_cmd="$(get_ai_interactive_cmd) 'context.md'"
  _open_session_layout "$name" "$session_dir" "$ai_cmd" "$codex_cmd"
...
# Build the codex exec command string for Reviewer B.
# Returns empty string if codex is not installed.
# Customize the prompt: ~/.bufo/prompts/reviewer-b.md
# Note: the prompt is collapsed to a single line (newlines → spaces) because
# codex exec takes the prompt as a single shell argument.
_build_reviewer_b_cmd() {
  command_exists codex || { echo ""; return 0; }
  local default='You are Reviewer B in a code review court. Read context.md for the full PR diff. Review independently for bugs, security issues, and code quality. Structure findings as Critical/Warning/Suggestion with file:line references. Save your complete findings to codex-review.md when done.'
  local prompt
  prompt=$(load_prompt "reviewer-b" "$default")
  local inline
  inline=$(printf '%s' "$prompt" | tr '\n' ' ')
  # printf '%q' produces a safely shell-quoted token — handles single quotes,
  # backslashes, and any other special characters in the prompt text.
  local quoted_inline
  quoted_inline=$(printf '%q' "$inline")
  echo "codex exec --full-auto --skip-git-repo-check ${quoted_inline}"
}
...
handle_review_command() {
  ...
  case "$cmd" in
    "")
      ...
      echo "  bufo chorus <PR>        Court review (Judge + 2 reviewers, thorough)"
...
# Handle court command (shortcut for bufo chorus <PR>)
...
handle_court_command() {
  ...
  case "$cmd" in
    "")
      ...
      echo ""
      echo "Commands:"
      echo "  bufo chorus <PR>        Quick court review for single PR"
      echo "  bufo chorus new         Interactive mode (multiple PRs + context)"
      echo "  bufo chorus ls          List court sessions"
      echo "  bufo chorus resume      Resume a court session"
      echo "  bufo chorus show        View saved court output"
      echo "  bufo chorus delete      Delete a court session"
      return 1
      ;;
    "new")
      court_new_interactive
