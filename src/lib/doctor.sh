#!/usr/bin/env bash
# bufo - doctor diagnostics

show_doctor() {
  echo -e "${CYAN} ◎ , ◎${NC}"
  echo -e "${CYAN}（ -──）  Bufo Doctor${NC}"
  echo -e "${CYAN}/(    )${NC}\\"
  echo ""
  ...
  if config_exists; then
    ...
    echo ""
    echo "Run 'bufo init' to create a config file."
    return
  fi
  ...
  if [ "$env_files_count" -gt 0 ] && [ -d "$WORKSPACE_BASE" ]; then
    ...
    if [ $fixable_envs -gt 0 ]; then
      ...
      echo ""
      echo -e "  ${YELLOW}Run 'bufo doctor --fix' to sync missing .env files${NC}"
    fi
  fi
...
# Fix tadpole issues (sync missing .env files)
...
doctor_fix() {
  ...
  if [ "$force" = "true" ] && [ $fixed -gt 0 ]; then
    ...
    echo "All tadpole .env files are up to date."
  else
