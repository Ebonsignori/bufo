#!/usr/bin/env bash
# bufo - git worktree operations, submodules, shared volume

# Get the branch name for tadpole N
get_branch_name() {
  local num=$1
  echo "${BRANCH_PATTERN//\{N\}/$num}"
}

# Create tadpole directory with git worktree
create_workspace() {
  local num=$1
  local workspace_dir="$TADPOLE_BASE/$TADPOLE_PREFIX-$num"
  ...
  setup_companions "$workspace_dir"
  ...
  if [ -n "$install_cmd" ]; then
    ...
    if run_install_command "$install_cmd" "$install_env"; then
      touch "$workspace_dir/node_modules/.bufo-installed"
    ...
    for ((i=0; i<submodules_count; i++)); do
      ...
      if [ -d "$workspace_dir/$sub_path" ]; then
        ...
        if run_install_command "$sub_install" "$install_env" "$sub_path"; then
          [ -d "node_modules" ] && touch "node_modules/.bufo-installed"
        fi
        cd "$workspace_dir"
      fi
    done
  ...
  touch "$workspace_dir/.bufo-lock"

  success "Workspace $num created at $workspace_dir"
}

# Initialize submodules in tadpole
init_submodules() {
  local dir=$1
  ...
  for ((i=0; i<submodules_count; i++)); do
    ...
    if [ -n "$reset_to" ] && [ "$reset_to" != "null" ] && [ -d "$dir/$sub_path" ]; then
      ...
      local _sub_default
      _sub_default=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's|refs/remotes/origin/||') || _sub_default="main"
      git checkout "$_sub_default" 2>/dev/null || true
...
reset_submodules() {
  ...
  for ((i=0; i<submodules_count; i++)); do
    ...
    if [ -d "$dir/$sub_path" ]; then
      ...
      _sub_default=$(git symbolic-ref refs/remotes/origin/HEAD 2>/dev/null | sed 's|refs/remotes/origin/||') || _sub_default="main"
      git checkout "$_sub_default" 2>/dev/null || true
      git fetch origin "$_sub_default" --quiet 2>/dev/null || true
      if [ -n "$reset_to" ] && [ "$reset_to" != "null" ]; then
        git reset --hard "$reset_to"
      else
        git reset --hard "origin/$_sub_default"
      fi
      cd "$dir"
...
# Setup shared volume symlink in a tadpole
setup_shared_volume() {
  local dir=$1
  ...
  if [ -f "$gitignore" ]; then
    if ! grep -q "^${link_name}$" "$gitignore" 2>/dev/null; then
      echo "" >> "$gitignore"
      echo "# Shared local volume (bufo)" >> "$gitignore"
...
show_shared() {
  ...
  if [ -d "$SHARED_VOLUME_PATH" ]; then
    ...
    echo -e "  ${YELLOW}Not yet created (will be created on first tadpole open)${NC}"
  fi
}
