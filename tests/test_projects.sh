...
session_name: alpha
tadpole_base: /tmp/alpha-tadpoles
...
session_name: beta
tadpole_base: /tmp/beta-tadpoles
...
# Create tadpole dirs to match
mkdir -p /tmp/alpha-tadpoles/alpha-tp-1
mkdir -p /tmp/beta-tadpoles/beta-tp-1

_orig_dir=$(pwd)

CONFIG_FILE=""
PROJECT_ALIAS=""
cd /tmp/alpha-tadpoles/alpha-tp-1
resolve_project_from_cwd
run_test "resolve_from_cwd matches alpha tadpole_base" "[ '$PROJECT_ALIAS' = 'alpha' ]"

CONFIG_FILE=""
PROJECT_ALIAS=""
cd /tmp/beta-tadpoles/beta-tp-1
resolve_project_from_cwd
run_test "resolve_from_cwd matches beta tadpole_base" "[ '$PROJECT_ALIAS' = 'beta' ]"
...
cd "$_orig_dir"
rmdir /tmp/alpha-tadpoles/alpha-tp-1 /tmp/alpha-tadpoles /tmp/beta-tadpoles/beta-tp-1 /tmp/beta-tadpoles 2>/dev/null || true
...
run_test "resolve_command_aliases finds 's'" "[ '$(resolve_command_aliases 's')' = 'tp 1' ]"
...
tadpole_base: $PROJ_TEST_DIR/dm-tadpoles
main_repo: $PROJ_TEST_DIR/dm-main
tadpoles:
  prefix: dm-tp
  branch_pattern: "dm-tp-{N}"
EOF
...
tadpole_base: $PROJ_TEST_DIR/dp-tadpoles
main_repo: $PROJ_TEST_DIR/dp-main
tadpoles:
  prefix: dp-tp
  branch_pattern: "dp-tp-{N}"
EOF
cat > "$GLOBAL_CONFIG" << EOF
...
# --- Test: delete removes tadpole directories ---
mkdir -p "$PROJ_TEST_DIR/tp-main"
git -C "$PROJ_TEST_DIR/tp-main" init -q 2>/dev/null
# Create a fake worktree directory (not a real worktree, rm -rf fallback handles it)
mkdir -p "$PROJ_TEST_DIR/tp-tadpoles/tpdel-1"
mkdir -p "$PROJ_TEST_DIR/tp-tadpoles/tpdel-2"

cat > "$PROJECTS_DIR/tpdel.yaml" << EOF
session_name: tpdel
tadpole_base: $PROJ_TEST_DIR/tp-tadpoles
main_repo: $PROJ_TEST_DIR/tp-main
tadpoles:
  prefix: tpdel
  branch_pattern: "tpdel-{N}"
EOF

output=$(echo -e "y\ntpdel" | remove_project "tpdel" 2>&1)
run_test "delete removes tadpole dirs" "[ ! -d '$PROJ_TEST_DIR/tp-tadpoles/tpdel-1' ] && [ ! -d '$PROJ_TEST_DIR/tp-tadpoles/tpdel-2' ]"
rm -rf "$PROJ_TEST_DIR/tp-main"

# --- Test: delete removes state directory ---
mkdir -p "$CONFIG_DIR/state/statedel"
echo '{}' > "$CONFIG_DIR/state/statedel/tp1.json"

cat > "$PROJECTS_DIR/statedel.yaml" << EOF
session_name: statedel
tadpole_base: $PROJ_TEST_DIR/sd-tadpoles
main_repo: $PROJ_TEST_DIR/sd-main
tadpoles:
  prefix: sd-tp
  branch_pattern: "sd-tp-{N}"
EOF
...
session_name: keepme
tadpole_base: $PROJ_TEST_DIR/km-tadpoles
...
session_name: wipdel
tadpole_base: $PROJ_TEST_DIR/wd-tadpoles
main_repo: $PROJ_TEST_DIR/wd-main
tadpoles:
  prefix: wd-tp
  branch_pattern: "wd-tp-{N}"
EOF
