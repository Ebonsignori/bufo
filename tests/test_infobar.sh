...
TADPOLE_BASE="$INFOBAR_TEST_DIR"
WORKSPACE_BASE="$INFOBAR_TEST_DIR"  # alias for compat
...
run_test "write_workspace_meta creates .bufo-meta" "[ -f '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta' ]"
run_test "Meta file has correct type" "[ \"\$(jq -r '.type' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'pr' ]"
run_test "Meta file has PR number" "[ \"\$(jq -r '.pr_number' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = '42' ]"
run_test "Meta file has PR URL" "[ \"\$(jq -r '.pr_url' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'https://github.com/owner/repo/pull/42' ]"
run_test "Meta file has PR title" "[ \"\$(jq -r '.pr_title' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'Fix login' ]"
run_test "Meta file has name" "[ \"\$(jq -r '.name' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'PR #42: Fix login' ]"

# Ticket metadata
write_workspace_meta 2 "ticket" "ticket" "ENG-456" "name" "ENG-456"
run_test "Ticket meta has correct type" "[ \"\$(jq -r '.type' '$TADPOLE_BASE/$TADPOLE_PREFIX-2/.bufo-meta')\" = 'ticket' ]"
run_test "Ticket meta has ticket ID" "[ \"\$(jq -r '.ticket' '$TADPOLE_BASE/$TADPOLE_PREFIX-2/.bufo-meta')\" = 'ENG-456' ]"
run_test "Ticket meta has name" "[ \"\$(jq -r '.name' '$TADPOLE_BASE/$TADPOLE_PREFIX-2/.bufo-meta')\" = 'ENG-456' ]"

# Overwrite metadata
write_workspace_meta 1 "ticket" "ticket" "PROJ-789" "name" "PROJ-789"
run_test "Overwritten meta has new type" "[ \"\$(jq -r '.type' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'ticket' ]"
run_test "Overwritten meta has new ticket" "[ \"\$(jq -r '.ticket' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'PROJ-789' ]"
run_test "Overwritten meta lost old PR field" "[ \"\$(jq -r '.pr_number // \"none\"' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'none' ]"
...
run_test "Meta includes ticket" "[ \"\$(jq -r '.ticket' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'ENG-555' ]"
run_test "Meta includes ticket_url" "[ \"\$(jq -r '.ticket_url' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = 'https://linear.app/team/issue/ENG-555' ]"
run_test "Meta includes pr_number" "[ \"\$(jq -r '.pr_number' '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta')\" = '99' ]"
...
run_test "render_infobar produces output" "[ -n \"\$(render_infobar '$TADPOLE_BASE/$TADPOLE_PREFIX-1')\" ]"
run_test "render_infobar shows PR number" "render_infobar '$TADPOLE_BASE/$TADPOLE_PREFIX-1' | grep -q '99'"
run_test "render_infobar shows ticket" "render_infobar '$TADPOLE_BASE/$TADPOLE_PREFIX-1' | grep -q 'ENG-555'"
run_test "render_infobar with no meta shows default" "render_infobar '$TADPOLE_BASE/$TADPOLE_PREFIX-3' | grep -q 'bufo'"

# Render ticket-only tadpole
run_test "render_infobar for ticket tadpole" "render_infobar '$TADPOLE_BASE/$TADPOLE_PREFIX-2' | grep -q 'ENG-456'"
...
run_test "clear_workspace_meta removes .bufo-meta" "! [ -f '$TADPOLE_BASE/$TADPOLE_PREFIX-1/.bufo-meta' ]"
run_test "clear_workspace_meta on missing file is no-op" "clear_workspace_meta 99"
run_test "render_infobar after clear shows default" "render_infobar '$TADPOLE_BASE/$TADPOLE_PREFIX-1' | grep -q 'bufo'"
