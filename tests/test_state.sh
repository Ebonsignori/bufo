#!/usr/bin/env bash
# Tests for tadpole state persistence (save/load/remove)
...
STATE_FILE="$STATE_TEST_DIR/state/testsession/tp1.json"
...
run_test "state_load sets TP_WINDOW_ID" "[ '$TP_WINDOW_ID' = 'win-123' ]"
run_test "state_load sets TP_TAB_ID" "[ '$TP_TAB_ID' = 'tab-456' ]"
run_test "state_load sets TP_TERMINAL_SID" "[ '$TP_TERMINAL_SID' = 'term-sid' ]"
run_test "state_load sets TP_SERVER_SID" "[ '$TP_SERVER_SID' = 'srv-sid' ]"
run_test "state_load sets TP_MAIN_SID" "[ '$TP_MAIN_SID' = 'main-sid' ]"
run_test "state_load sets TP_INFO_SID" "[ '$TP_INFO_SID' = 'info-sid' ]"
...
if has_jq; then
  STATE_FILE2="$STATE_TEST_DIR/state/testsession/tp2.json"
...
run_test "state_list finds tp1" "state_list_workspaces 'testsession' | grep -q '1'"
run_test "state_list finds tp2" "state_list_workspaces 'testsession' | grep -q '2'"
...
run_test "state_remove deletes file" "! [ -f '$STATE_TEST_DIR/state/testsession/tp1.json' ]"
...
run_test "state_workspace_exists cleans stale state" "! [ -f '$STATE_TEST_DIR/state/testsession/tp3.json' ]"

# =============================================================================
# Legacy state file fallback Tests
# =============================================================================

# Write a legacy ws<N>.json and verify it can be read back
mkdir -p "$STATE_TEST_DIR/state/legacysession"
cat > "$STATE_TEST_DIR/state/legacysession/ws4.json" << 'LEGACY_EOF'
{"workspace":4,"window_id":"win-legacy","tab_id":"tab-legacy","panes":{"terminal":"t-leg","server":"s-leg","main":"m-leg"},"created_at":"2024-01-01T00:00:00Z"}
LEGACY_EOF

state_load_workspace "legacysession" "4"
run_test "state_load falls back to legacy ws<N>.json" "[ '$TP_WINDOW_ID' = 'win-legacy' ]"
run_test "state_load legacy sets TP_TERMINAL_SID" "[ '$TP_TERMINAL_SID' = 't-leg' ]"
