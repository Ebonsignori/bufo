...
if has_yq; then
  ...
  PORT_TP_DIR="$PORT_TEST_DIR/tadpole"
  mkdir -p "$PORT_TP_DIR"
...
PORTEOF
  ...
  cat > "$PORT_TP_DIR/.env" << 'ENVEOF'
API_PORT=3200
APP_PORT=3000
OTHER_VAR=hello
ENVEOF

  run_test "read_env_port reads plain number" "[ '$(read_env_port "$PORT_TP_DIR" "api")' = '3200' ]"
...
ENVEOF
  ...
  run_test "read_env_port reads URL port" "[ '$(read_env_port "$PORT_TP_DIR" "api")' = '4200' ]"

  # Test: no .env file
  rm "$PORT_TP_DIR/.env"
  run_test "read_env_port empty with no .env" "[ -z '$(read_env_port "$PORT_TP_DIR" "api")' ]"
...
ENVEOF
  ...
  run_test "read_env_port empty when var missing" "[ -z '$(read_env_port "$PORT_TP_DIR" "api")' ]"
...
ENVEOF
  ...
  run_test "read_env_port strips spaces" "[ '$(read_env_port "$PORT_TP_DIR" "api")' = '3300' ]"
