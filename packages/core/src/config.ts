...
export function getBufoDir(): string { return BUFO_DIR; }
export function bufoExists(): boolean { return existsSync(BUFO_DIR); }
...
export function loadProject(alias: string, filePath: string): BufoProject {
  ...
  const tadpoles = (doc.tadpoles as Record<string, unknown>) || (doc.workspaces as Record<string, unknown>) || {};
  const ports = (doc.ports as Record<string, unknown>) || {};
  return {
    alias,
    session_name: (doc.session_name as string) || alias,
    tadpole_base: expandPath((doc.tadpole_base as string) || (doc.workspace_base as string) || ''),
    ...
    ports: ports ? { api_base: ports.api_base as number | undefined, app_base: ports.app_base as number | undefined } : undefined,
...
export function discoverProjects(): BufoProject[] {
  ...
  for (const file of files) {
    ...
    try { projects.push(loadProject(alias, join(PROJECTS_DIR, file))); } catch { /* skip */ }
  }
  const defaultAlias = loadGlobalConfig().default_project;
  if (defaultAlias) projects.sort((a, b) => (a.alias === defaultAlias ? -1 : b.alias === defaultAlias ? 1 : 0));
  return projects;
}
...
export function loadTadpoleState(sessionName: string, num: number): TadpoleState | undefined {
  ...
  if (!existsSync(stateFile)) stateFile = join(STATE_DIR, sessionName, `ws${num}.json`);
  if (!existsSync(stateFile)) return undefined;
  try { return JSON.parse(readFileSync(stateFile, 'utf-8')) as TadpoleState; } catch { return undefined; }
}
...
export function loadTadpoleMeta(tadpoleDir: string): TadpoleMeta | undefined {
  ...
  try { return JSON.parse(readFileSync(metaFile, 'utf-8')) as TadpoleMeta; } catch { return undefined; }
}
...
export function getCustomName(tadpoleDir: string): string | undefined {
  ...
  try { return readFileSync(nameFile, 'utf-8').trim() || undefined; } catch { return undefined; }
}
...
export function loadSession(
  ...
  try {
    ...
    if (existsSync(layoutFile)) {
      try { layout = JSON.parse(readFileSync(layoutFile, 'utf-8')) as SessionLayout; } catch { /* stale */ }
    }
    ...
    return {
      ...
      active, hasReviewOutput, layout,
    };
  } catch { return undefined; }
}

export function discoverSessions(projectAlias: string, activeSessions?: Set<string>): BufoSession[] {
  const projectSessionsDir = join(SESSIONS_DIR, projectAlias);
  if (!existsSync(projectSessionsDir)) return [];
  let entries: string[];
  try { entries = readdirSync(projectSessionsDir); } catch { return []; }
...
export function getAllSessions(activeSessions?: Set<string>): { projectAlias: string; sessions: BufoSession[] }[] {
  ...
  for (const project of projects) {
    ...
    if (sessions.length > 0) result.push({ projectAlias: project.alias, sessions });
  }
  return result;
}
