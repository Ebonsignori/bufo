...
// src/new-tadpole.tsx
var new_tadpole_exports = {};
__export(new_tadpole_exports, {
  default: () => NewTadpole
});
module.exports = __toCommonJS(new_tadpole_exports);
        ...
        import_api.Action.SubmitForm,
        {
          ...
          onSubmit: async (values) => {
            ...
            try {
              let cmd;
              if (values.source) {
                const isPr = values.source.includes("github.com") && values.source.includes("/pull/") || /^\d+$/.test(values.source.trim());
                cmd = isPr ? `@${values.project} pr ${values.source}` : `@${values.project} ticket ${values.source}`;
              } else if (values.number) {
                cmd = `@${values.project} tp ${values.number}`;
              } else {
                cmd = `@${values.project} spawn${values.reuse ? " --reuse" : ""}`;
              }
              await (0, import_api.showToast)({ style: import_api.Toast.Style.Animated, title: "Creating tadpole..." });
              await runBufoAsync(cmd);
              await (0, import_api.showToast)({ style: import_api.Toast.Style.Success, title: "Tadpole created" });
