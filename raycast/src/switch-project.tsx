...
import { discoverTadpoles } from "./lib/bufo";
...
function loadData() {
  ...
  return projects.map((project) => {
    const tadpoles = discoverTadpoles(project, activeSessions);
    const activeCount = tadpoles.filter((w) => w.active).length;
    const totalCount = tadpoles.length;
    return {
      project,
      activeCount,
      totalCount,
          ...
          accessories={[
            {
              ...
              tooltip: `${activeCount} active of ${totalCount} tadpoles`,
            },
            ...
            <ActionPanel>
              <Action
                ...
                onAction={async () => {
                  ...
                  try {
                    await runBufoAsync(`@${project.alias} main`);
              ...
              {!isDefault && (
                <Action
                  ...
                  onAction={async () => {
                    try {
                      await runBufoAsync(`default ${project.alias}`);
                      await showToast({
